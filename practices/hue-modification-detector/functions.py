...
def crosscorr(array1, array2):
    ...
    # End of filtering
    normalizator = np.sqrt(np.sum(np.power(array1, 2))
                           * np.sum(np.power(array2, 2)))
...
def NoiseExtract(Im, qmf, sigma, L):
    ...
    nr = (np.ceil((M+minpad)/m)*m).astype(np.int)
    # dimensions of the padded image (always pad 8 pixels or more)
    nc = (np.ceil((N+minpad)/m)*m).astype(np.int)
    # number of padded rows on the top
    pr = np.ceil((nr-M)/2).astype(np.int)
    # number of padded rows at the bottom
    prd = np.floor((nr-M)/2).astype(np.int)
    # number of padded columns on the left
    pc = np.ceil((nc-N)/2).astype(np.int)
    # number of padded columns on the right
    pcr = np.floor((nc-N)/2).astype(np.int)
    Im = np.block([
        [Im[pr-1::-1, pc-1::-1],       Im[pr-1::-1, :],
            Im[pr-1::-1, N-1:N-pcr-1:-1]],
        [Im[:, pc-1::-1],              Im,                   Im[:, N-1:N-pcr-1:-1]],
        [Im[M-1:M-prd-1:-1, pc-1::-1], Im[M-1:M-prd-1:-1, :],
            Im[M-1:M-prd-1:-1, N-1:N-pcr-1:-1]]
    ])

    # Precompute noise variance and initialize the output
    NoiseVar = sigma**2
    # Wavelet decomposition, without redudance
    wave_trans = filters.mdwt(Im, qmf, L)
    ...
    for i in range(1, L+1):
        ...
        wave_trans[0:nr//2, nc//2:nc], _ = \
            filters.WaveNoise(wave_trans[0:nr//2, nc//2:nc], NoiseVar)

        # Vertical noise extraction
        wave_trans[nr//2:nr, 0:nc//2], _ = \
            filters.WaveNoise(wave_trans[nr//2:nr, 0:nc//2], NoiseVar)

        # Diagonal noise extraction
        wave_trans[nr//2:nr, nc//2:nc], _ = \
            filters.WaveNoise(wave_trans[nr//2:nr, nc//2:nc], NoiseVar)
    ...
    image_noise = filters.midwt(wave_trans, qmf, L)
...
def Saturation(X, gray=False):
    ...
    if np.ndim(np.squeeze(X)) == 3:
        ...
        maxX = X.max()
        SaturMap = np.logical_not(np.logical_and(
            X == maxX, np.logical_not(SaturMap)))
    else:
...
def ZeroMean(X, zType='CFA'):
    ...
    if zType == 'col':
        for j in range(K):
            Y[:, :, j] = X[:, :, j] - np.tile(col[j, :], (M, 1))
    elif zType == 'row':
        for j in range(K):
            Y[:, :, j] = X[:, :, j] - np.tile(row[:, j], (N, 1)).transpose()
    elif zType == 'both':
        for j in range(K):
            Y[:, :, j] = X[:, :, j] - np.tile(col[j, :], (M, 1))
        for j in range(K):
            # equal to Y = ZeroMean(X,'row'); Y = ZeroMean(Y,'col');
            Y[:, :, j] = X[:, :, j] - np.tile(row[:, j], (N, 1)).transpose()
    elif zType == 'CFA':
        for j in range(K):
            Y[:, :, j] = X[:, :, j] - np.tile(col[j, :], (M, 1))
        for j in range(K):
            # equal to Y = ZeroMean(X,'both');
            Y[:, :, j] = X[:, :, j] - np.tile(row[:, j], (N, 1)).transpose()
        for j in range(K):
...
def PCE(C, shift_range=[0, 0], squaresize=11):
    ...
    if any(np.greater_equal(shift_range, C.shape)):
        # all possible shift in at least one dimension
        shift_range = min(shift_range, C.shape-1)

    shift_range = np.array(shift_range)
    Out = dict(PCE=[], pvalue=[], PeakLocation=[],
               peakheight=[], P_FA=[], log10P_FA=[])
    ...
    # C[-1,-1] location corresponds to no shift of the first matrix argument of 'crosscor2'
    Cinrange = C[-1-shift_range[0]:, -1-shift_range[1]:]
    [max_cc, imax] = np.max(Cinrange.flatten()), np.argmax(Cinrange.flatten())
    [ypeak, xpeak] = np.unravel_index(imax, Cinrange.shape)[
        0], np.unravel_index(imax, Cinrange.shape)[1]
    ...
    # p-value
    # under simplifying assumption that C are samples from Gaussian pdf
    Out['pvalue'] = 1/2 * \
        special.erfc(Out['peakheight']/np.sqrt(PCE_energy)/np.sqrt(2))
    [Out['P_FA'], Out['log10P_FA']] = _FAfromPCE(
        Out['PCE'], np.prod(shift_range+1))
