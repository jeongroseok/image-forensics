...
def WaveNoise(coef, NoiseVar):
    ...
    tc = np.power(coef, 2)
    coefVar = Threshold(
        signal.fftconvolve(tc, np.ones(
            [3, 3], dtype=np.float32) / (3 * 3), mode='same'),
        NoiseVar)

    for w in range(5, 9 + 1, 2):
        EstVar = Threshold(
            signal.fftconvolve(tc, np.ones(
                [w, w], dtype=np.float32) / (w * w), mode='same'),
            NoiseVar)
...
def NoiseExtractFromImage(image, sigma=3.0, color=False, noZM=False):
    ...
    if three != 3:
        Noise = functions.NoiseExtract(X, qmf, sigma, L)
    else:
        Noise = np.zeros(X.shape)
        for j in range(3):
            Noise[:, :, j] = functions.NoiseExtract(X[:, :, j], qmf, sigma, L)
        if not color:
            Noise = functions.rgb2gray1(Noise)
    if noZM:
        print('not removing the linear pattern')
    else:
        Noise, _ = functions.ZeroMeanTotal(Noise)
        ...
        for actual_L in range(1, L+1):
            ...
            for ir in range(actual_m):  # loop over rows
                ...
                if actual_L == 1:
                    ...
                    # from LL of previous level
                    xdummy[:actual_n] = y[ir, :actual_n]
                # perform filtering lowpass and highpass
                xdummy, ydummyl, ydummyh = _fpsconv(
                    xdummy, actual_n, h0, h1, lhm1, ydummyl, ydummyh)
            ...
            if _m > 1:  # in case of a 2D signal
                ...
                for ic in range(actual_n):  # loop over column
                    ...
                    # perform filtering lowpass and highpass
                    xdummy, ydummyl, ydummyh = _fpsconv(
                        xdummy, actual_m, h0, h1, lhm1, ydummyl, ydummyh)
        ...
        for actual_L in range(L, 0, -1):
            ...
            if _m > 1:
                ...
                for ic in range(actual_n):  # loop over column
                    ...
                    # perform filtering lowpass and highpass
                    xdummy = _bpsconv(xdummy, r_o_a, g0, g1,
                                      lhhm1, ydummyl, ydummyh)
                    # restore dummy variables in matrix
            ...
            for ir in range(actual_m):  # loop over rows
                ...
                # perform filtering lowpass and highpass
                xdummy = _bpsconv(xdummy, c_o_a, g0, g1,
                                  lhhm1, ydummyl, ydummyh)
                # restore dummy variables in matrices
